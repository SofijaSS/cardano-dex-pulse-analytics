...
`vercel.json` registers one daily cache warm-up at `07:15 UTC`. It calls `/api/cron/refresh-dashboard`, never bypasses dashboard authentication for a user, and returns only cache metadata. Vercel automatically supplies `Authorization: Bearer <CRON_SECRET>` when that environment variable exists. The final normalized DEX snapshot refreshes hourly, while price, current-source, and benchmark adapters retain their own 5-minute, hourly, and 12-hour policies.

The Sites/Cloudflare deployment uses D1 for one durable last-known-good dashboard snapshot. A request returns that snapshot immediately. When it is at least one hour old, the Worker schedules a background refresh with a cross-worker lease so only one refresh runs at a time. A failed or unusable refresh records the error without replacing the last successful payload. The first request after an empty database still performs one synchronous source load to seed D1.
...
Provider payloads are cached separately through Next's cache API. ADA/USD providers refresh every `PRICE_REFRESH_SECONDS` (default 5 minutes); current native DEX and TVL providers refresh every `DEX_REFRESH_SECONDS` (default 60 minutes); daily and historical benchmark providers refresh every `BENCHMARK_REFRESH_SECONDS` (default 12 hours). The normalized DEX response is additionally stored as a durable D1 snapshot with a default one-hour refresh interval. Failed background revalidation keeps the last successful payload available, while its original source timestamps allow the UI to mark provider data stale instead of presenting it as new. Token-chart snapshots remain independent and refresh every 5 minutes.
...
Browser
  -> GET /api/dashboard
     -> durable 60m last-known-good D1 snapshot
        -> return immediately when present
        -> refresh expired data in the background with a D1 lease
...
lib/dashboard-snapshot.ts   durable snapshot orchestration and background refresh
lib/dashboard-snapshot-store.ts D1 persistence and cross-worker refresh lease

...
| Minswap official Market Data API | `POST https://api-mainnet-prod.minswap.org/v1/pools/metrics`; request the documented `protocols=["Minswap","MinswapV2","MinswapStable"]` with `only_verified=true`, follow every `search_after` cursor, and sum `volume_24h`, `volume_7d`, `trading_fee_24h`, `trading_fee_7d`, and `liquidity_currency`. Omitted `currency` returns ADA and is primary. A parallel USD 24h response is reconciliation-only. `pool_metrics[].type` supplies the exact deployment. | Rolling/current; app stale threshold: 2h. | Public, keyless and documented in `minswap/skills`. The table represents complete paginated verified-pool coverage, not unverified pool activity. Native 30d and previous 7d are unavailable. Provider USD values are not used for display when they disagree with native ADA and the independent ADA/USD price. Pagination fails closed on duplicate pools, repeated cursors, or more than 2,000 verified pools. |
| Minswap Market Insights (SundaeSwap only) | `GET https://api-internal.minswap.org/api/v1/market/dex-analytic?timeframe=1M` and `timeframe=6M`; match exact `protocol[]` IDs `sundae-cpmm-v1`, `sundae-cpmm-v3`, and `sundae-stable-cpmm-v1`, then use the same index in `vol`, `tvl`, `fee`, `trade`, and `awallet`. `timestamp[]` supplies each UTC daily bucket. The feeds are merged by timestamp with the fresher `1M` value winning; `6M` only supplies older missing buckets. `vol`, `tvl`, and `fee` are USD; 24h is the latest complete UTC bucket, while 7d, previous 7d, and 30d are validated sums of 7, 7, and 30 complete buckets. The active partial UTC day is excluded from every period. | Daily complete UTC buckets; app stale threshold: 26h. Persistent source snapshot refresh: 1h. | Public and keyless, but undocumented and without a published SLA. The current frontend/backend implementation is not present in Minswap's public GitHub repositories; the endpoint and mapping were verified from the live Market Insights requests and rendered values. V1 and V3 stay exact; Stable contributes only to the SundaeSwap family total. `awallet` maps to DAU for one exact contract series, but is not summed across contracts because wallets can overlap. |
...
| Fees 24h / 7d | Minswap `trading_fee_24h` / `trading_fee_7d`; aggregate WingRiders `dailyFees` for 24h; PoolFlow period fees only for WingRiders V1; Minswap Market Insights daily `fee` buckets for exact SundaeSwap V1/V3 rows. Other DEXes show `Data unavailable`. |
...
The main table shows one canonical row per independently tracked DEX version. `Minswap` means the original V1 contract, while `Minswap V2` is the current V2 contract. `WingRiders` is the legacy V1 row sourced only from PoolFlow's exact version row, while `WingRiders V2` is the primary current deployment mapped from the official protocol feed with an explicit source note. `SundaeSwap V1` and `SundaeSwap V3` now use their exact Minswap Market Insights contract series. Sundae Stable remains part of the family total used once in aggregate cards and charts, but is not renamed or displayed as V3. Minswap Stable and duplicate family-total rows are not shown. Family totals remain available in each row's expandable source details without being counted twice.
...
- Minswap ADA is primary. Its USD response is reconciliation-only: when the implied rate differs by more than 5% from the fresh ADA/USD reference, provider USD is discarded and dashboard USD is calculated from the native ADA value and independent price.
...
- Minswap pagination covers every verified pool returned by its documented Market Data API for the three supported Minswap deployments; unverified pools and non-Minswap DEXes are outside that metric's stated coverage.

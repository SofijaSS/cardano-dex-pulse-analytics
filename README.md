...
| Minswap Market Insights (SundaeSwap only) | `GET https://api-internal.minswap.org/api/v1/market/dex-analytic?timeframe=6M`; match exact `protocol[]` IDs `sundae-cpmm-v1`, `sundae-cpmm-v3`, and `sundae-stable-cpmm-v1`, then use the same index in `vol`, `tvl`, `fee`, `trade`, and `awallet`. `timestamp[]` supplies each UTC daily bucket. `vol`, `tvl`, and `fee` are USD; 24h is the latest daily bucket, while 7d, previous 7d, and 30d are validated sums of 7, 7, and 30 buckets. | Daily complete UTC buckets; app stale threshold: 26h. Persistent source snapshot refresh: 1h. | Public and keyless, but undocumented and without a published SLA. The current frontend/backend implementation is not present in Minswap's public GitHub repositories; the endpoint and mapping were verified from the live Market Insights requests and rendered values. V1 and V3 stay exact; Stable contributes only to the SundaeSwap family total. `awallet` maps to DAU for one exact contract series, but is not summed across contracts because wallets can overlap. |

...
| Minswap Market Insights (Minswap, SundaeSwap and CSWAP) | `GET https://api-internal.minswap.org/api/v1/market/dex-analytic?timeframe=1M` and `timeframe=6M`; match exact `protocol[]` IDs `minswap-cpmm-v1`, `minswap-cpmm-v2`, `minswap-stable-cpmm-v1`, `sundae-cpmm-v1`, `sundae-cpmm-v3`, and `sundae-stable-cpmm-v1`. CSWAP accepts only source IDs whose normalized value starts with `cswap`; the complete CSWAP ID set must agree between both feeds. The same protocol index is used in `vol`, `tvl`, `fee`, `trade`, and `awallet`. `timestamp[]` supplies each UTC daily bucket. The feeds are merged by timestamp with the fresher `1M` value winning; `6M` only supplies older missing buckets. `vol`, `tvl`, and `fee` are USD; 24h is the latest complete UTC bucket, while 7d, previous 7d, and 30d are validated sums of complete buckets. The active partial UTC day is excluded. Displayed freshness is the derived end of the latest complete bucket (`timestamp + 24h`). | Daily complete UTC buckets; app stale threshold: 26h. Persistent source snapshot refresh: 1h. | Primary period source for exact Minswap V1/V2 and SundaeSwap V1/V3 rows, plus the CSWAP family row. Public and keyless, but undocumented and without a published SLA. Minswap's documented aggregator identifies CSWAP as `CswapV1`; Market Insights currently displays the CSWAP family without a version split. CSWAP components are summed only after cross-feed identity agreement. `awallet` is not summed across contracts because wallets can overlap. |
...
| Fees 24h / 7d | Minswap Market Insights daily `fee` buckets for exact Minswap V1/V2, SundaeSwap V1/V3, and CSWAP V1 rows; the official Minswap pool API is the rolling fallback. Aggregate WingRiders `dailyFees` supplies 24h; PoolFlow period fees apply only to WingRiders V1. Other DEXes show `Data unavailable`. |
| Trades, Users, DAU | PoolFlow exact 24h deployment rows supply these fields for Minswap V1/V2, WingRiders V1/V2, Splash and snek.fun. They are an auxiliary layer and do not replace official DEX volume. Minswap Market Insights maps the latest exact-contract `trade` and `awallet` buckets to SundaeSwap V1/V3 and CSWAP V1. Family-level active wallets remain unavailable when multiple contracts can contain the same wallet; `Users` remains unavailable because this source labels only daily active wallets. |
...
The main table shows one canonical row per independently tracked DEX version. `Minswap` means the exact `minswap-cpmm-v1` series, while `Minswap V2` uses `minswap-cpmm-v2`; both come from Minswap Market Insights complete daily buckets. `WingRiders` is the legacy V1 row sourced only from PoolFlow's exact version row, while `WingRiders V2` is the primary current deployment mapped from the official protocol feed with an explicit source note. `SundaeSwap V1` and `SundaeSwap V3` use their exact Market Insights contract series. `CSWAP` represents the complete set of source protocols whose normalized IDs start with `cswap`, and is used only after the `1M` and `6M` feeds agree on that exact set. Stable deployments remain part of their family totals used once in aggregate cards and charts, but are not renamed as V2/V3 table rows. Family totals remain available in expandable source details without being counted twice.

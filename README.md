...
The main table shows one canonical row per independently tracked DEX version. `Minswap` means the exact `minswap-cpmm-v1` series, while `Minswap V2` uses `minswap-cpmm-v2`; both come from Minswap Market Insights complete daily buckets. `WingRiders` is the legacy V1 row sourced only from PoolFlow's exact version row, while `WingRiders V2` is the primary current deployment mapped from the official protocol feed with an explicit source note. `SundaeSwap V1` and `SundaeSwap V3` use their exact Market Insights contract series. `CSWAP` represents the complete set of source protocols identified as CSWAP V1 only after the `1M` and `6M` feeds agree on that set. Stable deployments remain part of their family totals used once in aggregate cards and charts, but are not renamed as V2/V3 table rows. Family totals remain available in expandable source details without being counted twice.

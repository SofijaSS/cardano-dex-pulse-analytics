...
| Minswap official Market Data API | `POST https://api-mainnet-prod.minswap.org/v1/pools/metrics`; request the documented `protocols=["Minswap","MinswapV2","MinswapStable"]` with `only_verified=true`, follow every `search_after` cursor, and sum `volume_24h`, `volume_7d`, `trading_fee_24h`, `trading_fee_7d`, and `liquidity_currency`. Omitted `currency` returns ADA and is primary within this source. A parallel USD 24h response is reconciliation-only. `pool_metrics[].type` supplies the exact deployment. | Rolling/current; app stale threshold: 2h. | Public, keyless and documented in `minswap/skills`. Used as a transparent rolling fallback and pool-count source because its rolling pool sum does not share the complete-UTC-day semantics of Market Insights. Provider USD is discarded when it disagrees with native ADA and the independent ADA/USD price. Pagination fails closed on duplicate pools, repeated cursors, or more than 2,000 verified pools. |
...
| Pools | Number of distinct verified Minswap pool rows returned after complete documented pagination, by version; SundaeSwap aggregate `stats.poolCount`. The column is labelled as observed coverage and is not assumed complete across providers. |
...
- Minswap Market Insights period buckets are USD-denominated and converted to ADA only for display using the dashboard's timestamped price. For the official rolling pool fallback, ADA is primary and its provider-USD response is reconciliation-only; a mismatch above 5% discards provider USD.
...
- Minswap period reporting uses exact Market Insights contract IDs and complete UTC buckets. The separately displayed rolling pool fallback covers every verified pool returned by the documented Market Data API; the two definitions are not averaged because their windows differ.

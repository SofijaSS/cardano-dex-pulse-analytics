...
| Minswap official Market Data API | `POST https://api-mainnet-prod.minswap.org/v1/pools/metrics`; request the documented `protocols=["Minswap","MinswapV2","MinswapStable"]`, follow every `search_after` cursor, and sum `volume_24h`, `volume_7d`, `trading_fee_24h`, `trading_fee_7d`, and `liquidity_currency`. Omitted `currency` returns ADA and is primary. A parallel USD 24h response is reconciliation-only. `pool_metrics[].type` supplies the exact deployment. | Rolling/current; app stale threshold: 2h. | Public, keyless and documented in `minswap/skills`. Native 30d and previous 7d are unavailable. Provider USD values are not used for display when they disagree with native ADA and the independent ADA/USD price. Pagination fails closed on duplicate pools, repeated cursors, or more than 2,000 pools. |
...
| Pools | Number of distinct Minswap pool rows returned after complete documented pagination, by version; SundaeSwap aggregate `stats.poolCount`. The column is labelled as observed coverage and is not assumed complete across providers. |
...
- Minswap pagination covers every pool returned by its documented Market Data API for the three supported Minswap deployments; this does not imply coverage of non-Minswap DEXes.

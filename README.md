...
| Minswap Market Insights (Minswap and SundaeSwap) | `GET https://api-internal.minswap.org/api/v1/market/dex-analytic?timeframe=1M` and `timeframe=6M`; match exact `protocol[]` IDs `minswap-cpmm-v1`, `minswap-cpmm-v2`, `minswap-stable-cpmm-v1`, `sundae-cpmm-v1`, `sundae-cpmm-v3`, and `sundae-stable-cpmm-v1`, then use the same index in `vol`, `tvl`, `fee`, `trade`, and `awallet`. `timestamp[]` supplies each UTC daily bucket. The feeds are merged by timestamp with the fresher `1M` value winning; `6M` only supplies older missing buckets. `vol`, `tvl`, and `fee` are USD; 24h is the latest complete UTC bucket, while 7d, previous 7d, and 30d are validated sums of 7, 7, and 30 complete buckets. The active partial UTC day is excluded. | Daily complete UTC buckets; app stale threshold: 26h. Persistent source snapshot refresh: 1h. | Primary period source for exact Minswap V1/V2 and SundaeSwap V1/V3 rows. Public and keyless, but undocumented and without a published SLA. The protocol IDs and matrix mapping were verified against live requests used by Minswap's Market Insights page. Stable contributes only to its family total. `awallet` is not summed across contracts because wallets can overlap. |

...
Import the GitHub repository into Vercel as a Next.js project and configure the shared-login values from **Optional shared login** as encrypted Production, Preview, and Development environment variables. Also set `NEXT_PUBLIC_SITE_URL` to the final HTTPS deployment URL and add a random `CRON_SECRET` of at least 16 characters. Connect a private Vercel Blob store with its read/write token exposed as `BLOB_READ_WRITE_TOKEN`. Do not commit `.env.local`, the password hash, the Blob token, the cron secret, or the session-signing secret.

`vercel.json` registers one daily cache warm-up at `07:15 UTC` plus two Wednesday weekly-report slots at `06:00` and `07:00 UTC`. The weekly endpoint accepts only the slot that resolves to Wednesday `08:00 Europe/Belgrade`, so daylight-saving changes do not move the local reporting cutoff. Cron routes never bypass dashboard authentication for a user and return only cache/snapshot metadata. Vercel automatically supplies `Authorization: Bearer <CRON_SECRET>` when that environment variable exists. Vercel Hobby cron jobs have hourly precision, so the invocation can happen during the `08:00–08:59` local window; exact-minute execution requires Vercel Pro or a separate minute-precision scheduler. The final normalized DEX snapshot uses Vercel's persistent Data Cache with a one-hour stale-while-revalidate interval, while price, current-source, and benchmark adapters retain their own 5-minute, hourly, and 12-hour policies.

### Wednesday reporting snapshots

Reconciled `7d volume`, `Previous 7d`, `WoW`, 7-day rank, the weekly brief, and the weekly comparison chart use durable Wednesday snapshots rather than provider-relative rolling comparisons. At Wednesday `08:00 Europe/Belgrade`, the current normalized 7-day value is frozen in both ADA and USD. It remains the displayed current 7-day value until the next Wednesday cutoff; on that next cutoff it becomes `Previous 7d`, and WoW is calculated from the two saved Wednesday values. Current 24-hour data, TVL, and other live observations retain their normal refresh cadence.

Cloudflare Sites stores structured weekly snapshots in D1 (`DB`, table `weekly_reporting_snapshots`). The Vercel deployment uses one private Blob object, `cardano-dex-pulse/weekly-reporting-state.json`, as the durable source of truth. Both adapters retain exactly two immutable slots: `current` and `previous`. A new Wednesday capture atomically moves `current` to `previous`, writes the new `current`, and discards anything older; duplicate invocations in the same week preserve the first capture. A missed scheduled capture is filled on the first dashboard request and marked `late`; older missing weeks are never reconstructed from a later rolling value. A reviewed seed may fill only a missing `null` metric in the matching current snapshot and can never replace an existing number. The 22 and 29 July 2026 rows are explicitly seeded from the archived 29 July presentation table, the 5 August row is backfilled from its archived presentation table, and the 12 August 2026 row is seeded from the production snapshot captured at `08:08:15 CEST` before the durable-store migration. Its missing Dano Finance 7d value is reconstructed from the official `dailyVolumeAdaValue` records for the seven complete UTC days from 5 through 11 August.
...
| Minswap Market Insights (Minswap, SundaeSwap, CSWAP and Splash) | `GET https://api-internal.minswap.org/api/v1/market/dex-analytic?timeframe=1M` and `timeframe=6M`; match exact `protocol[]` IDs `minswap-cpmm-v1`, `minswap-cpmm-v2`, `minswap-stable-cpmm-v1`, `sundae-cpmm-v1`, `sundae-cpmm-v3`, and `sundae-stable-cpmm-v1`. CSWAP and Splash accept only source IDs whose normalized values start with `cswap` and `splash`; each complete family ID set must agree between both feeds. The same protocol index is used in `vol`, `tvl`, `fee`, `trade`, and `awallet`. `timestamp[]` supplies each UTC daily bucket. The feeds are merged by timestamp with the fresher `1M` value winning; `6M` only supplies older missing buckets. `vol`, `tvl`, and `fee` are USD; 24h is the latest complete UTC bucket, while 7d, previous 7d, and 30d are validated sums of complete buckets. The active partial UTC day is excluded. Displayed freshness is the derived end of the latest complete bucket (`timestamp + 24h`). | Daily complete UTC buckets; app stale threshold: 26h. Persistent source snapshot refresh: 1h. | Primary period source for exact Minswap V1/V2 and SundaeSwap V1/V3 rows, plus CSWAP and Splash family rows. Public and keyless, but undocumented and without a published SLA. Market Insights currently displays both families without version splits. Components are summed only after cross-feed identity agreement. `awallet` is not summed across contracts because wallets can overlap. |
| CSWAP official TVL contract registry | CSWAP's official DefiLlama adapter identifies the DEX pool, orderbook, and staking addresses and uses Cardano `sumTokens2` for TVL/staking. The dashboard uses those published identities as reconciliation provenance while the current TVL value comes from the protocol-indexed Market Insights row, with DefiLlama retained as the independent benchmark. | On-chain/current through provider indexing. | The official CSWAP repository does not expose period volume, trades, users, or DAU. Its adapter proves contract scope, not historical trading metrics. |
...
| Splash official analytics | `GET https://analytics.splash.trade/platform-api/v1/platform/stats`; `volumeUsd / 1e6`, `tvlUsd / 1e6` | Rolling/current; app stale threshold: 2h. | Independent current-period check and fallback when the Minswap Splash family index is unavailable. It does not expose verified 7d, 30d, previous 7d, fees, trades, users, or DAU fields. |
...
| DeltaDeFi | `GET .../public/volume/daily?timestamp=...`; `volume_usd` | Latest complete UTC day; app stale threshold: 50h from the period-start timestamp. Dedicated 4s timeout and one attempt. | The endpoint and response shape still match the current DefiLlama adapter, but the DeltaDeFi API origin returned Cloudflare 522 during the 2026-07-29 review. The last successful persistent snapshot remains eligible through stale-while-revalidate; without one, DeltaDeFi is explicitly marked as a known limitation and excluded from active source coverage. DefiLlama history is accepted only after live agreement passes. |
...
Dashboard and token source responses use structural validation, a configurable 7-second timeout, and two attempts with exponential backoff. DeltaDeFi uses its documented source-specific fail-fast policy. Timeout failures are reported as source timeouts rather than leaking the underlying abort signal. Non-retryable 4xx responses fail immediately. Each source reports `healthy`, `stale`, or `error`. Auth is checked before every API response; browser responses remain private and `no-store` while login protection is enabled.

Provider payloads are cached separately in Next's persistent Data Cache, which Vercel shares across requests and deployments. The DefiLlama protocol response is projected to Cardano-only records before caching so unrelated multi-chain metadata does not consume the Vercel cache budget. ADA/USD providers refresh every `PRICE_REFRESH_SECONDS` (default 5 minutes); current native DEX and TVL providers refresh every `DEX_REFRESH_SECONDS` (default 60 minutes); daily and historical benchmark providers refresh every `BENCHMARK_REFRESH_SECONDS` (default 12 hours). The normalized DEX response also has its own persistent one-hour snapshot plus an in-instance single-flight layer. After the first successful seed, expired snapshots are served stale while Vercel refreshes them in the background. Manual refresh keeps DeltaDeFi's last successful provider payload under its source-specific stale-while-revalidate tag; all other adapters preserve their existing immediate refresh behavior. Failed DeltaDeFi revalidation keeps that snapshot available, while its original source timestamp allows the UI to mark it stale instead of presenting it as new. Token-chart snapshots remain independent and refresh every 5 minutes.
...
| --- | --- |
| 24h / 7d / 30d volume, previous 7d, WoW, TVL, volume/TVL, share, rank | 24h and 30d retain the native-first reconciliation described above. Reconciled 7d, previous 7d and WoW use durable Wednesday 08:00 Europe/Belgrade snapshots. Version rows remain separate from protocol totals. |
| Fees 24h / 7d | Minswap Market Insights daily `fee` buckets for exact Minswap V1/V2 and SundaeSwap V1/V3 rows, plus CSWAP and Splash family rows; the official Minswap pool API is the rolling fallback. Aggregate WingRiders `dailyFees` supplies 24h; PoolFlow period fees apply only to WingRiders V1. Other DEXes show `Data unavailable`. |
| Trades, Users, DAU | PoolFlow exact 24h deployment rows supply these fields for Minswap V1/V2, WingRiders V1/V2, Splash and snek.fun. They are an auxiliary layer and do not replace period DEX volume. Minswap Market Insights maps the latest exact-contract `trade` and `awallet` buckets to SundaeSwap V1/V3, CSWAP and Splash. Family-level active wallets remain unavailable when multiple contracts can contain the same wallet; `Users` remains unavailable because this source labels only daily active wallets. |
...
The main table shows one canonical row per independently tracked DEX version. `Minswap` means the exact `minswap-cpmm-v1` series, while `Minswap V2` uses `minswap-cpmm-v2`; both come from Minswap Market Insights complete daily buckets. `WingRiders` is the legacy V1 row sourced only from PoolFlow's exact version row, while `WingRiders V2` is the primary current deployment mapped from the official protocol feed with an explicit source note. `SundaeSwap V1` and `SundaeSwap V3` use their exact Market Insights contract series. `CSWAP` and `Splash` represent complete source-protocol families whose normalized IDs start with `cswap` and `splash`, and are used only after the `1M` and `6M` feeds agree on the exact family set. Stable deployments remain part of their family totals used once in aggregate cards and charts, but are not renamed as V2/V3 table rows. Family totals remain available in expandable source details without being counted twice.
...
- CSWAP's official repository publishes TVL contract addresses but no period-volume API. Minswap's documented aggregator identifies `CswapV1`, and the separate Market Insights feed supplies the CSWAP family's daily protocol-indexed metrics. If its CSWAP-prefixed protocol set is missing or differs between history and recent responses, every CSWAP Market Insights metric remains unavailable.
- Splash's official GitHub organization publishes protocol SDKs and a DefiLlama TVL fork but no documented whole-DEX period-volume API. Minswap Market Insights supplies the Splash family's daily protocol-indexed metrics; the official Splash analytics endpoint remains the current 24h/TVL fallback. If the Splash-prefixed protocol set is missing or differs between history and recent responses, every Splash Market Insights metric remains unavailable.

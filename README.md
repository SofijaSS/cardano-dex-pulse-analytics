...
| SundaeSwap official API | `POST https://api.sundae.fi/graphql`; `stats.volume.quantity` for lovelace, `stats.poolCount`, and `protocols[].version` | Current protocol metadata; app stale threshold: 2h. | Retained for protocol/version confirmation and aggregate pool count. Its aggregate, unclearly labelled volume window is only a fallback when Minswap Market Insights is unavailable; it is never used to manufacture a V1/V3 split. Native `stats.tvl` remains excluded because its semantics did not reconcile with protocol TVL. |
...
| Trades, Users, DAU | PoolFlow's exact 24h `WingRiders` row supplies its period fields. Minswap Market Insights maps the latest exact-contract `trade` and `awallet` buckets to SundaeSwap V1/V3 trades and DAU. SundaeSwap family-level wallets are left unavailable rather than double-counting a wallet that used multiple contracts; its `Users` field remains unavailable because the source only labels daily active wallets. |
